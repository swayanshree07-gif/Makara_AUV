// generated from rosidl_generator_c/resource/idl__functions.c.em
// with input from hammerhead_interfaces:msg/VectorNavData.idl
// generated code does not contain a copyright notice
#include "hammerhead_interfaces/msg/detail/vector_nav_data__functions.h"

#include <assert.h>
#include <stdbool.h>
#include <stdlib.h>
#include <string.h>

#include "rcutils/allocator.h"


// Include directives for member types
// Member `header`
#include "std_msgs/msg/detail/header__functions.h"
// Member `orientation`
// Member `acceleration`
#include "rosidl_runtime_c/primitives_sequence_functions.h"

bool
hammerhead_interfaces__msg__VectorNavData__init(hammerhead_interfaces__msg__VectorNavData * msg)
{
  if (!msg) {
    return false;
  }
  // header
  if (!std_msgs__msg__Header__init(&msg->header)) {
    hammerhead_interfaces__msg__VectorNavData__fini(msg);
    return false;
  }
  // orientation
  if (!rosidl_runtime_c__float__Sequence__init(&msg->orientation, 0)) {
    hammerhead_interfaces__msg__VectorNavData__fini(msg);
    return false;
  }
  // acceleration
  if (!rosidl_runtime_c__float__Sequence__init(&msg->acceleration, 0)) {
    hammerhead_interfaces__msg__VectorNavData__fini(msg);
    return false;
  }
  return true;
}

void
hammerhead_interfaces__msg__VectorNavData__fini(hammerhead_interfaces__msg__VectorNavData * msg)
{
  if (!msg) {
    return;
  }
  // header
  std_msgs__msg__Header__fini(&msg->header);
  // orientation
  rosidl_runtime_c__float__Sequence__fini(&msg->orientation);
  // acceleration
  rosidl_runtime_c__float__Sequence__fini(&msg->acceleration);
}

bool
hammerhead_interfaces__msg__VectorNavData__are_equal(const hammerhead_interfaces__msg__VectorNavData * lhs, const hammerhead_interfaces__msg__VectorNavData * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // header
  if (!std_msgs__msg__Header__are_equal(
      &(lhs->header), &(rhs->header)))
  {
    return false;
  }
  // orientation
  if (!rosidl_runtime_c__float__Sequence__are_equal(
      &(lhs->orientation), &(rhs->orientation)))
  {
    return false;
  }
  // acceleration
  if (!rosidl_runtime_c__float__Sequence__are_equal(
      &(lhs->acceleration), &(rhs->acceleration)))
  {
    return false;
  }
  return true;
}

bool
hammerhead_interfaces__msg__VectorNavData__copy(
  const hammerhead_interfaces__msg__VectorNavData * input,
  hammerhead_interfaces__msg__VectorNavData * output)
{
  if (!input || !output) {
    return false;
  }
  // header
  if (!std_msgs__msg__Header__copy(
      &(input->header), &(output->header)))
  {
    return false;
  }
  // orientation
  if (!rosidl_runtime_c__float__Sequence__copy(
      &(input->orientation), &(output->orientation)))
  {
    return false;
  }
  // acceleration
  if (!rosidl_runtime_c__float__Sequence__copy(
      &(input->acceleration), &(output->acceleration)))
  {
    return false;
  }
  return true;
}

hammerhead_interfaces__msg__VectorNavData *
hammerhead_interfaces__msg__VectorNavData__create(void)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  hammerhead_interfaces__msg__VectorNavData * msg = (hammerhead_interfaces__msg__VectorNavData *)allocator.allocate(sizeof(hammerhead_interfaces__msg__VectorNavData), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(hammerhead_interfaces__msg__VectorNavData));
  bool success = hammerhead_interfaces__msg__VectorNavData__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
hammerhead_interfaces__msg__VectorNavData__destroy(hammerhead_interfaces__msg__VectorNavData * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    hammerhead_interfaces__msg__VectorNavData__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
hammerhead_interfaces__msg__VectorNavData__Sequence__init(hammerhead_interfaces__msg__VectorNavData__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  hammerhead_interfaces__msg__VectorNavData * data = NULL;

  if (size) {
    data = (hammerhead_interfaces__msg__VectorNavData *)allocator.zero_allocate(size, sizeof(hammerhead_interfaces__msg__VectorNavData), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = hammerhead_interfaces__msg__VectorNavData__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        hammerhead_interfaces__msg__VectorNavData__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
hammerhead_interfaces__msg__VectorNavData__Sequence__fini(hammerhead_interfaces__msg__VectorNavData__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      hammerhead_interfaces__msg__VectorNavData__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

hammerhead_interfaces__msg__VectorNavData__Sequence *
hammerhead_interfaces__msg__VectorNavData__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  hammerhead_interfaces__msg__VectorNavData__Sequence * array = (hammerhead_interfaces__msg__VectorNavData__Sequence *)allocator.allocate(sizeof(hammerhead_interfaces__msg__VectorNavData__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = hammerhead_interfaces__msg__VectorNavData__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
hammerhead_interfaces__msg__VectorNavData__Sequence__destroy(hammerhead_interfaces__msg__VectorNavData__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    hammerhead_interfaces__msg__VectorNavData__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
hammerhead_interfaces__msg__VectorNavData__Sequence__are_equal(const hammerhead_interfaces__msg__VectorNavData__Sequence * lhs, const hammerhead_interfaces__msg__VectorNavData__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!hammerhead_interfaces__msg__VectorNavData__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
hammerhead_interfaces__msg__VectorNavData__Sequence__copy(
  const hammerhead_interfaces__msg__VectorNavData__Sequence * input,
  hammerhead_interfaces__msg__VectorNavData__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(hammerhead_interfaces__msg__VectorNavData);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    hammerhead_interfaces__msg__VectorNavData * data =
      (hammerhead_interfaces__msg__VectorNavData *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!hammerhead_interfaces__msg__VectorNavData__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          hammerhead_interfaces__msg__VectorNavData__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!hammerhead_interfaces__msg__VectorNavData__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}
