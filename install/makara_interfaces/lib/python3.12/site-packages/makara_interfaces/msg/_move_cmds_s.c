...
bool makara_interfaces__msg__move_cmds__convert_from_py(PyObject * _pymsg, void * _ros_message)
{
  // check that the passed message is of the expected Python class
  {
    ...
    assert(strncmp("makara_interfaces.msg._move_cmds.MoveCmds", full_classname_dest, 41) == 0);
  }
  makara_interfaces__msg__MoveCmds * ros_message = _ros_message;
  {  // len
  ...
  {  // list
    ...
    if (!makara_interfaces__msg__MoveCmd__Sequence__init(&(ros_message->list), size)) {
      PyErr_SetString(PyExc_RuntimeError, "unable to create makara_interfaces__msg__MoveCmd__Sequence ros_message");
      Py_DECREF(seq_field);
      Py_DECREF(field);
      return false;
    }
    makara_interfaces__msg__MoveCmd * dest = ros_message->list.data;
    for (Py_ssize_t i = 0; i < size; ++i) {
      if (!makara_interfaces__msg__move_cmd__convert_from_py(PySequence_Fast_GET_ITEM(seq_field, i), &dest[i])) {
        Py_DECREF(seq_field);
        Py_DECREF(field);
        return false;
      }
...
PyObject * makara_interfaces__msg__move_cmds__convert_to_py(void * raw_ros_message)
{
  ...
  {
    PyObject * pymessage_module = PyImport_ImportModule("makara_interfaces.msg._move_cmds");
  ...
  {  // list
    ...
    for (size_t i = 0; i < size; ++i) {
      ...
      PyObject * pyitem = makara_interfaces__msg__move_cmd__convert_to_py(item);
      if (!pyitem) {
        Py_DECREF(field);
        return NULL;
      }
